      ...
      <div className="flex items-center px-8 ps-3 pe-3 bg-[#2e2e30]  text-white shadow-sm py-3">
        ...
        <div className="w-full flex justify-end items-center gap-4  z-[1]">
          ...
          <div className="relative">
            ...
            {/* message box  */}
            {isChatBoxOpen && (
              <div className=" absolute -right-28 top-14 shadow-2xl z-10 bg-white overflow-y-scroll rounded-lg h-[590px] w-[350px] px-2 py-4">
                <div className="text-black px-2">
                    ...
                    <div className="relative">
                      <button
                        ...
                        className={`flex items-center gap-1 ${
                          isAddChatOpen ? "bg-purple-100 " : "bg-gray-100"
                        } transition  shadow hover:shadow  px-3 text-sm py-1 rounded-full`}
                      >
                        <p>Add</p>
                        <AiOutlinePlus />
                      </button>
                      ...
                      <dialog id="my_modal_3" className="modal">
                        <div className="modal-box">
                          <form method="dialog">
                            <button
                              onClick={() => setIsAddChatOpen(!isAddChatOpen)}
                                ...
                                return (
                                  <div
                                    ...
                                    onClick={() => {
                                      ...
                                      setIsMessageBoxOpen(!isMessageBoxOpen);
                                      setAddNewChat(singleUser?.email)
                                      setIsChatBoxOpen(!isChatBoxOpen);
                                    }}
                  ...
                  <div className="relative ">
                    <input
                      onChange={(e) => handleCoversationSearch(e)}
                      onFocus={handleInputFocus}
                      onBlur={handleInputBlur}
                      type="text"
                      placeholder="Search users"
                  ...
                  userMessagesList
                    ...
                    .map((message) => {
                      ...
                      const lastMessageTimeStamp =
                        message?.lastMassages?.timestamp;
                      ...
                      const lastMsg =
                        message?.lastMassages?.text.length <= 17
                          ? message?.lastMassages?.text
                          : message?.lastMassages?.text.slice(0, 17) + "...";

                      const senderStatus =
                        user.email == message?.lastMassages?.sender && "You: ";
                      const reciverUserName = message?.participants
                        ?.find((participant) => participant != user?.email)
                        ?.match(/[^@]*/)[0]
                        ?.split(".")[0];

                      const unseenMessage =
                        user.email == message?.lastMassages?.receiver;

                      const seenMessageReceiver =
                        message?.lastMassages?.receiver;
                      ...
                      return (
                        ...
                        >
                          <div className="relative">
                            ...
                            {activeUsers?.find(
                              (active) => active?.email == message?.participants?.find(participant => participant != user?.email) 
                            ) && (
                              <p className="bg-green-500 w-2 h-2 rounded-full absolute right-3 bottom-[7px]"></p>
                            )}
                          </div>
                          <div>
                            ...
                            <div className="flex gap-2">
                              <p
                                className={` ${
                                  unseenMessage && !message.isMessageSeen
                              ...
                              <p className="text-gray-600 text-sm flex items-center">
                                <BsDot size={10} className="p-0 m-0" />{" "}
                                {agoTime}
                              </p>
                            </div>
                          </div>
